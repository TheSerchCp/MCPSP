...
void StateOptions::Init()
{
	...
	menuoptions = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::menuOptions));
    menuoptions->SetPosition(255,129);
		...
		case 0:
		{
			...
			if(mSystemMgr->KeyPressed(PSP_CTRL_CROSS))
			{
				if(selectPos == 0)
				{
					...
					{
						mSoundMgr->LoadSonds();
						//mTextureMgr->UnLoadTexture();
						mSoundMgr->Ram32 = false;
					}
					else
					{
						mSoundMgr->UnLoadSonds();
						mSoundMgr->Ram32 = true;
						//mTextureMgr->UnLoadTexture();
					}
				}
				if(selectPos == 1)
...
void StateOptions::Draw(StateManager* sManager)
{
	...
	switch(menuState)
	{
		case 0:
		{
			...
			//selected button
			sbuttonSprite->SetPosition(240,(selectPos * 40) + 80);
			sbuttonSprite->Draw();
            ...
            {
				if (mSoundMgr->Ram32 == true){selectPos == 0 ? DrawText(240,88,GU_COLOR(1,1,1,1) ,default_size,"Available Ram: 32mb") : DrawText(240,88,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Available Ram: 32mb");}else{selectPos == 0 ? DrawText(240,88,GU_COLOR(1,1,1,1) ,default_size,"Available Ram: 64mb") : DrawText(240,88,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Available Ram: 64mb");}
                selectPos == 1 ? DrawText(240,129,GU_COLOR(1,1,1,1) ,default_size,"Controls") : DrawText(240,129,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Controls");
                selectPos == 2 ? DrawText(240,169,GU_COLOR(1,1,1,1) ,default_size,"Analog stick") : DrawText(240,169,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Analog stick");
                selectPos == 3 ? DrawText(240,209,GU_COLOR(1,1,1,1) ,default_size,"Cancel") : DrawText(240,209,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Cancel");

                DrawText(240,40,GU_COLOR(1,1,1,1) ,default_size,"Options");
            }
            if(mRender->GetFontLanguage() == RUSSIAN)
            {
				if (mSoundMgr->Ram32 == true){selectPos == 0 ? DrawText(240,88,GU_COLOR(1,1,1,1) ,default_size,"Ram Disponible: 32mb") : DrawText(240,88,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Ram Disponible: 32mb");}else{selectPos == 0 ? DrawText(240,88,GU_COLOR(1,1,1,1) ,default_size,"Ram Disponible: 64mb") : DrawText(240,88,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Ram Disponible: 64mb");}
                selectPos == 1 ? DrawText(240,129,GU_COLOR(1,1,1,1) ,default_size,"Controles") : DrawText(240,129,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Controles");
                selectPos == 2 ? DrawText(240,169,GU_COLOR(1,1,1,1) ,default_size,"Mando Analogico") : DrawText(240,169,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Mando Analogico");
                selectPos == 3 ? DrawText(240,209,GU_COLOR(1,1,1,1) ,default_size,"Regresar") : DrawText(240,209,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Regresar");

                DrawText(240,40,GU_COLOR(1,1,1,1) ,default_size,"Opciones");
            }
		}
		break;
		case 1://controls
		{
			...
			{
				//action
				controlPos == i ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000000) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000000);
			...
			for(int i = controlStart;i < controlEnd;i++)
			{
				...
				controlPos == i ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
            ...
            if(mRender->GetFontLanguage() == ENGLISH)
            {
                controlPos == 16 ? DrawText(240,269,GU_COLOR(1,1,1,1) ,default_size,"Cancel") : DrawText(240,269,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Cancel");
                DrawText(240,40,GU_COLOR(1,1,1,1) ,default_size,"Controls");
            }
            if(mRender->GetFontLanguage() == RUSSIAN)
            {
                controlPos == 16 ? DrawText(240,269,GU_COLOR(1,1,1,1) ,default_size,"Regresar") : DrawText(240,269,GU_COLOR(0.25,0.25,0.25,1) ,default_size,"Regresar");
                DrawText(240,40,GU_COLOR(1,1,1,1) ,default_size,"Controles");
            }
		}
		break;
		case 2://analog stick
		{
			...
			moverSprite->SetPosition(130 +((fabs(InputHelper::Instance()->analogYup) * 10) * 22),100);
			moverSprite->Draw();

			moverSprite->SetPosition(130 +((fabs(InputHelper::Instance()->analogYdown) * 10) * 22),130);
			moverSprite->Draw();

			moverSprite->SetPosition(130 +((fabs(InputHelper::Instance()->analogXleft) * 10) * 22),160);
			moverSprite->Draw();

			moverSprite->SetPosition(130 +((fabs(InputHelper::Instance()->analogXright) * 10) * 22),190);
			moverSprite->Draw();
            ...
            if(mRender->GetFontLanguage() == ENGLISH)
            {
                currentAnalogPos == 0 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                mRender->DebugPrint(240,109,"Analog up   : %d%%",(int)(fabs(InputHelper::Instance()->analogYup) * 100.0f));

                currentAnalogPos == 1 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                mRender->DebugPrint(240,139,"Analog down : %d%%",(int)(fabs(InputHelper::Instance()->analogYdown) * 100.0f));

                currentAnalogPos == 2 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                mRender->DebugPrint(240,169,"Analog left : %d%%",(int)(fabs(InputHelper::Instance()->analogXleft) * 100.0f));

                currentAnalogPos == 3 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                mRender->DebugPrint(240,199,"Analog right: %d%%",(int)(fabs(InputHelper::Instance()->analogXright) * 100.0f));

                currentAnalogPos >= 4 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                mRender->DebugPrint(240,269,"Cancel");

                DrawText(240,75,GU_COLOR(1,1,1,1) ,default_size,"Analog stick");
            }
            if(mRender->GetFontLanguage() == RUSSIAN)
            {
                currentAnalogPos == 0 ? mRender->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00000200) : mRender->SetFontStyle(default_size,GU_COLOR(0.25,0.25,0.25,1),0,0x00000200);
                ...
                DrawText(240,75,GU_COLOR(1,1,1,1) ,default_size,"Mando analogico");
            }
		}
		break;
	}
