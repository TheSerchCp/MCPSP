...
Pig::Pig(Vector3 position1, int number1) : LameMob(position1, number1)
{
	...
	thepigTexture = TextureHelper::Instance()->GetTexture(TextureHelper::juanalbertoTexture);
	colorpig = 0;
...
void Pig::SetColorpig(int newcolorpig)
{
	colorpig = newcolorpig;
}
...
void Pig::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
            ...
            if(colorpig == 1){
				TextureManager::Instance()->SetTextureModeulate(thepigTexture);
			}
			else{
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc = {PIXEL*-2,PIXEL*-3,0};
            sceGumTranslate(&loc);
            ...
            sceGumPushMatrix();
            ScePspFVector3 loc2 = {PIXEL*11,PIXEL*2,0};
            ...
            {
                sceGumPushMatrix();
                ScePspFVector3 loc52 = {PIXEL*1.5f,PIXEL*6,0};
