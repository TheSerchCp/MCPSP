...
WitherSkeleton::WitherSkeleton(Vector3 position1, int number1) : LameMob(position1, number1)
{
    legs = Vector3(0,PIXEL*-6,0);

    SetCollideBox(Vector3(PIXEL*4,PIXEL*23,PIXEL*4),Vector3(PIXEL*4,PIXEL*12,PIXEL*4));	//y = 18
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::witherskeletonTexture);
}
...
void WitherSkeleton::BoxCollision()
{
    for(int x = 0; x < world->mWitherSkeletons.size(); x++)
    {
        WitherSkeleton *TestWitherSkeleton2 = world->mWitherSkeletons[x];
        if(TestWitherSkeleton2->number != number && (TestWitherSkeleton2->position.x != position.x || TestWitherSkeleton2->position.z != position.z || TestWitherSkeleton2->position.y != position.y))
        {
            if(TestWitherSkeleton2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestWitherSkeleton2->position.x,position.z-TestWitherSkeleton2->position.z); // super secret function
...
void WitherSkeleton::TakeDamage(float damage, float power, float dt)
{
    float AP = 2.0f;
...
void WitherSkeleton::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void WitherSkeleton::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
    ...
    if(toDestroy == false)
    {
        ...
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14+5)
        {
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
					if(lastKickType == 1 && fastDistanceToPlayer < 15 && world->HP > 0)
					{
						haunting = false;
                        hauntingTimer = 0.0f;
...
void WitherSkeleton::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
			...
			sceGumPushMatrix();
			ScePspFVector3 loc8 = {PIXEL*12,PIXEL*8,PIXEL*5};
			...
			world->drawFull3DItemFixed(256, false, 1.0f);
			sceGumPopMatrix();
