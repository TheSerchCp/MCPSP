...
Skeleton::Skeleton(Vector3 position1, int number1) : LameMob(position1, number1)
{
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::skeletonTexture);
...
void Skeleton::BoxCollision()
{
    for(int x = 0; x < world->mSkeletons.size(); x++)
    {
        Skeleton *TestSkeleton2 = world->mSkeletons[x];
        if(TestSkeleton2->number != number && (TestSkeleton2->position.x != position.x || TestSkeleton2->position.z != position.z || TestSkeleton2->position.y != position.y))
        {
            if(TestSkeleton2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestSkeleton2->position.x,position.z-TestSkeleton2->position.z); // super secret function
...
void Skeleton::TakeDamage(float damage, float power, float dt)
{
    float AP = 2.0f;
    AP = 2.0f;
...
void Skeleton::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Skeleton::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
    ...
    if(toDestroy == false)
    {
		...
		if (world->BlockSolid(position.x+0.125f,position.y,position.z) == true || world->BlockSolid(position.x-0.125f,position.y,position.z) == true || world->BlockSolid(position.x,position.y,position.z + 0.125f) == true || world->BlockSolid(position.x,position.y,position.z - 0.125f) == true)
		{
			if(world->BlockSolid(position.x+1.5f,position.y,position.z) == false)
			{
				position.x += 1;
			}
			else if(world->BlockSolid(position.x-1.5f,position.y,position.z) == false)
			{
				position.x -= 1;
			}
			else if(world->BlockSolid(position.x,position.y,position.z+1.5f) == false)
			{
				position.z += 1;
			}
			else if(world->BlockSolid(position.x,position.y,position.z-1.5f) == false)
        ...
        if(dt < 0.1f)
        {
            ...
            if(sunTimer > 1.5f)
            {
                if((world->worldDayTime > 6 && world->worldDayTime < 21 && world->GetShadowLevel(position.x,position.y-0.2f,position.z) == 0) && world->worldWeather->GetWeatherType() == 0)//clear weather, sun, Skeleton under clear sky
                {
                    HP -= 2.0f;
                    damaged = true;
                    damagedTimer = 0.9f;
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
                    ...
                    if(angred == true && kicked == false)
                    {
						...
						AnglePlayer = ATAN2_fast(position.x-relPlayerPos.x,position.z-relPlayerPos.z) + PI;
...
void Skeleton::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
			...
			// hand vertices
            sceGumPushMatrix();
            ScePspFVector3 loc3 = {-0.2,PIXEL*4,PIXEL*6};
			...
			// hand vertices
            sceGumPushMatrix();
            ScePspFVector3 loc4 = {-0.2,PIXEL*4,-PIXEL*4};
            sceGumTranslate(&loc4);

            sceGumRotateX(0.0f);
            sceGumRotateY(PI * 1.9);
            sceGumRotateZ(lHandAngle);
			...
			// leg vertices
            sceGumPushMatrix();
            ScePspFVector3 loc5 = {0,-PIXEL*6,PIXEL*3};
			...
			// leg vertices
            sceGumPushMatrix();
            ScePspFVector3 loc6 = {0,-PIXEL*6,-PIXEL*1};
			...
			// bow vertices
			// bow vertices ID = 408
			TextureManager::Instance()->SetTextureModeulate(world->textureItemsId);
			sceGumPushMatrix();
			ScePspFVector3 loc7 = {PIXEL*10.0,PIXEL*4.5,PIXEL*4.5};
			...
			ScePspFVector3 scale = {0.9,0.9,0.9};
			sceGumScale(&scale);
			
			world->drawFull3DItemFixed(408, false, 1.0f);
			sceGumPopMatrix();
