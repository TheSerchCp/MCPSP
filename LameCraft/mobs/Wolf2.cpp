...
Wolf::Wolf(Vector3 position1, int number1) : LameMob(position1, number1)
{
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::wolfTexture);
}
...
void Wolf::BoxCollision()
{
    for(int x = 0; x < world->mWolfs.size(); x++)
    {
        Wolf *TestWolf2 = world->mWolfs[x];
        if(TestWolf2->number != number && (TestWolf2->position.x != position.x || TestWolf2->position.z != position.z || TestWolf2->position.y != position.y))
        {
            if(TestWolf2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestWolf2->position.x,position.z-TestWolf2->position.z); // super secret function
...
void Wolf::TakeDamage(float damage, float power, float dt)
{
    float AP = 2.0f;
	...
	for (int i = 0; i < world->mWolfs.size(); i++)
    {
        Wolf* nearbyWolf = world->mWolfs[i];

        float dx = nearbyWolf->position.x - position.x;
        float dy = nearbyWolf->position.y - position.y;
        float dz = nearbyWolf->position.z - position.z;

        if ((dx * dx + dy * dy + dz * dz) < (35.0f * 35.0f))
        {
            nearbyWolf->atqgroup = true;
        }
    }
}

void Wolf::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Wolf::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
    ...
    if(toDestroy == false)
    {
        ...
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14+5)
        {
            ...
            if(dt < 0.1f)
            {
                ...
                if(killed == false)
                {
					if((lastKickType == 1 || atqgroup == true) && fastDistanceToPlayer < 15 && world->HP > 0)
					{
						haunting = false;
                        hauntingTimer = 0.0f;
                ...
                if(kicked == true)
                {
                    ...
                    if((walkingOnGround == true || footInWater) && damagedTimer <= 0.0f)
                    {
                        kicked = false;
                    }
...
void Wolf::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        if(fastDistanceToPlayer < world->mainOptions.horizontalViewDistance*14 && camFrustum.BoxInFrustum(bBox) == Frustum::Intersects)
        {
			...
			// LegN_Up vertices
            sceGumPushMatrix();
            ScePspFVector3 loc = {-PIXEL*5,0,-PIXEL*2.5};
            sceGumTranslate(&loc);
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->wolfLegVertices);
            sceGumPopMatrix();
			
			// LegN_Up vertices
            sceGumPushMatrix();
            ScePspFVector3 loc1 = {-PIXEL*5,0,PIXEL*0.5};
			...
			// LegN_Up vertices
            sceGumPushMatrix();
            ScePspFVector3 loc2 = {PIXEL*6,0,-PIXEL*2.5};
			...
			// LegN_Up vertices
            sceGumPushMatrix();
            ScePspFVector3 loc3 = {PIXEL*6,0,PIXEL*0.5};
			...
			// golden sword vertice
			// golden sword ID = 259
			TextureManager::Instance()->SetTextureModeulate(world->textureItemsId);
			sceGumPushMatrix();
			ScePspFVector3 loc8 = {PIXEL*9,PIXEL*8,PIXEL*5.5};
			...
			world->drawFull3DItemFixed(259, false, 1.0f);
			sceGumPopMatrix();*/
