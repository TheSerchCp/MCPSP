...
Pigman::Pigman(Vector3 position1, int number1) : LameMob(position1, number1)
{
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::pigmanTexture);
}
...
void Pigman::BoxCollision()
{
    for(int x = 0; x < world->mPigmans.size(); x++)
    {
        Pigman *TestPigman2 = world->mPigmans[x];
        if(TestPigman2->number != number && (TestPigman2->position.x != position.x || TestPigman2->position.z != position.z || TestPigman2->position.y != position.y))
        {
            if(TestPigman2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestPigman2->position.x,position.z-TestPigman2->position.z); // super secret function
...
void Pigman::TakeDamage(float damage, float power, float dt)
{
    float AP = 2.0f;
	...
	for (int i = 0; i < world->mPigmans.size(); i++)
    {
        Pigman* nearbyPigman = world->mPigmans[i];

        float dx = nearbyPigman->position.x - position.x;
        float dy = nearbyPigman->position.y - position.y;
        float dz = nearbyPigman->position.z - position.z;

        if ((dx * dx + dy * dy + dz * dz) < (35.0f * 35.0f))
        {
            nearbyPigman->atqgroup = true;
        }
    }
}

void Pigman::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Pigman::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
...
void Pigman::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
