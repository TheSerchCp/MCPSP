...
Villager::Villager(Vector3 position1, int number1) : LameMob(position1, number1)
{
    SPD = 1.0f;
    HP = 20.0f;
    ...
    SetCollideBox(Vector3(PIXEL*4,PIXEL*11,PIXEL*4),Vector3(PIXEL*4,PIXEL*12,PIXEL*4));
    ...
    mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::villagerTexture);
	mainTexture = TextureHelper::Instance()->GetTexture(TextureHelper::villagerTexture);
	butcher = TextureHelper::Instance()->GetTexture(TextureHelper::butcherTexture);
	farmer = TextureHelper::Instance()->GetTexture(TextureHelper::farmerTexture);
	librarian = TextureHelper::Instance()->GetTexture(TextureHelper::librarianTexture);
	priest = TextureHelper::Instance()->GetTexture(TextureHelper::priestTexture);
	smith = TextureHelper::Instance()->GetTexture(TextureHelper::smithTexture);
	
	Profession = 12;
	int ProfessionRandom = rand() % 6;
	if(ProfessionRandom == 0) Profession = rand()% 4;                
	if(ProfessionRandom == 1) Profession = 4 + rand()% 2;
	if(ProfessionRandom == 2) Profession = 6;
	if(ProfessionRandom == 3) Profession = 7 + rand()% 3;
	if(ProfessionRandom == 4) Profession = 10 + rand()% 2;
	if(ProfessionRandom == 5) Profession = 12;
...
//RETURN PROFESSION AFTER OF SAVE
void Villager::SetProfession(int newProfession)
{
...
void Villager::BoxCollision()
{
    for(int x = 0; x < world->mVillagers.size(); x++)
    {
        Villager *TestVillager2 = world->mVillagers[x];
        if(TestVillager2->number != number && (TestVillager2->position.x != position.x || TestVillager2->position.z != position.z || TestVillager2->position.y != position.y))
        {
            if(TestVillager2->bBox.intersect(bBox))
            {
                float angle1 = ATAN2_fast(position.x-TestVillager2->position.x,position.z-TestVillager2->position.z); // super secret function
...
void Villager::TakeDamage(float damage, float power, float dt)
{
    HP -= damage;
...
void Villager::TakeDamageFromPointExplosion(float damage, float power, float dt, Vector3 point)
{
    explosionPos = point;
...
void Villager::Update(CraftWorld *myWorld, Vector3 playerPos, float dt)
{
    world = myWorld;
...
void Villager::Render(Frustum &camFrustum, float dt)
{
    if(toDestroy == false)
    {
        ...
        {		
			...
			{
				TextureManager::Instance()->SetTextureModeulate(farmer);
			}
			else if(Profession >= 4 && Profession <= 5)
			{
				TextureManager::Instance()->SetTextureModeulate(librarian);
			}
			else if (Profession == 6)
			{
				TextureManager::Instance()->SetTextureModeulate(priest);
			}
			else if (Profession >= 7 && Profession <= 9)
			{
				TextureManager::Instance()->SetTextureModeulate(smith);
			}
			else if (Profession >= 10 && Profession <= 11)
			{
				TextureManager::Instance()->SetTextureModeulate(butcher);
			}
			...
			sceGumPushMatrix();
            ScePspFVector3 loc = {0,PIXEL*8,0};
            sceGumTranslate(&loc);
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerBodyVertices);
            sceGumPopMatrix();
			
			

            sceGumPushMatrix();
            ScePspFVector3 loc1 = {0,PIXEL*20,0};
            sceGumTranslate(&loc1);

            sceGumRotateX(0.0f);
            sceGumRotateY(headAngleY);
            sceGumRotateZ(0.0f);

            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerHeadVertices);
            // nose drawing
            if(DistanceToPlayer() < 40.0f)
            {
                sceGumPushMatrix();
                ScePspFVector3 loc2 = {0,PIXEL*3,0};
                sceGumTranslate(&loc2);

                sceGumRotateX(0.0f);
                sceGumRotateY(0.0f);
                sceGumRotateZ(0.0f);
                sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerNosesVertices);
                sceGumPopMatrix();
            }
			...
			//ArmR
			sceGumPushMatrix();
            ScePspFVector3 loc3 = {PIXEL*4,PIXEL*8, PIXEL*5.5};
            sceGumTranslate(&loc3);

            sceGumRotateX(0.0f);
            sceGumRotateY(0.0f);
            sceGumRotateZ(PI/4.186f);
			//PI/4.186f

            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerHandVertices);
            sceGumPopMatrix();
			
			//ArmL
			sceGumPushMatrix();
            ScePspFVector3 loc4 = {PIXEL*4,PIXEL*8, -PIXEL*5.5};
			...
			//ArmMiddle
			sceGumPushMatrix();
            ScePspFVector3 loc5 = {PIXEL*4,PIXEL*8, -PIXEL*2};
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerMidHandVertices);
            sceGumPopMatrix();

			
            //LegR
			sceGumPushMatrix();
            ScePspFVector3 loc6 = {0, 0, PIXEL*2};
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerLegVertices);
            sceGumPopMatrix();
			
			
			//LegL
			sceGumPushMatrix();
            ScePspFVector3 loc7 = {0, 0, -PIXEL*2};
			...
			//Body Wear
			sceGuEnable(GU_BLEND); //activar la transparencia
            ...
            sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF|GU_VERTEX_32BITF|GU_TRANSFORM_3D, 36, 0, world->villagerBodyWearVertices);
            sceGumPopMatrix();
