...
void WorldGenerator::initRandompMap(int worldSize, int worldHeight, int chunkSize, CraftWorld *world, bool makeDungeons, bool makeBonus, unsigned int seedII, int worldType, bool nether)
{
    ...
    if(worldType == DEFAULT)
    {
		...
		bool portalCreate = false;
		int glgen = 20 + rand()%16;
        ...
        for (int z = 0; z < WORLD_SIZE; z++)
        {
            for (int x = 0; x < WORLD_SIZE; x++)
            {
				...
				float HillFrequency = (GetValue2D(x,z,4,0.05f,1)+1.0f)/2.0f;

				// Obtener informacion de altura mediante ruido
				int Height;
				if(!nether){Height = (InterpolateRoughnessNoise(x,z) * GetValue2D3(x,z,4,0.1f,1))*64*HillFrequency+62;
				}else{Height = (GetValue2D3(x,z,4,0.1f,1))*64*HillFrequency+62;}
				

                if(Height <= 48) Height = 48-(48-Height)/2;
                if(Height >= 98) Height = 98+(Height-98)/2;
					...
					for (int y = Height; y >= 0; y--)
					{
						if(y == Height)
						{
							if(world->GetBlock(x,y+1,z) == 0)
							{
								if(rand() % 275 == 0)
								{
									world->GetBlock(x,y + 1,z) = FireBlock::getID();
									world->UpdateLightAreaIn(Vector3(x,y + 1,z));
								}
		...
		if(nether == false)
		{
			...
			if(makeDungeons)
			{
				...
				//initNetherFortress(WORLD_SIZE, world, 1);
			}
			...
			// Hacer aparecer animales para que el jugador pueda comenzar a gusto
			for(int i = 0; i < 6; i++){
				int spawnX, spawnY, spawnZ;
				int offset = (i < 4) ? 20 : 30;
				spawnX = offset + rand() % (WORLD_SIZE - offset);
				spawnZ = offset + rand() % (WORLD_SIZE - offset);
		...
		{
			initNehterEnvironmentBlocks(WORLD_SIZE, world, 1);
			initNehterEnvironmentBlocks(WORLD_SIZE, world, 2);
			initNehterEnvironmentBlocks(WORLD_SIZE, world, 3);
			initQuartz(WORLD_SIZE, world);
			initNetherFortress(WORLD_SIZE, world, 1);
			initBedrock(WORLD_SIZE, world);
		}
		
		// Sistema para spawnear al jugador
        int tries = 0;
        ...
        while(tries < 3072)
        {
            ...
            if(groundBlock != WaterBlock::getID() && groundBlock != Lava::getID())
            {
		...
		// Nether capa superior
		if(nether)
		{
			perlin->setSeed(seed+1030);
			...
			for (int z = 0; z < WORLD_SIZE; z++)
			{
				...
				{				
					...
					float HillFrequency = (GetValue2D(x,z,4,0.05f,1)+1.0f)/2.0f;
					int Height = (InterpolateRoughnessNoise(x,z) * GetValue2D3(x,z,4,0.1f,1))*64*HillFrequency+62;
					...
					for (int y = Height; y >= 0; y--)
					{
						if(y == Height)
						{	
							if(rand() % 875 == 0) //Glowstone
							{
								if(glgen > 0)
								{
									int glgen2 = 15 + rand()%16;
									for(int ggg2 = 0 ; ggg2 <= glgen2; ggg2++)
									{
										if(ggg2 == 0)
										{
											world->GetBlock(x + rand()%4,((110 - y) + 48)- rand()%4,z + rand()%4) = Glowstone::getID();
											world->UpdateLightAreaIn(Vector3(x + rand()%4,((110 - y) + 48)- rand()%4,z + rand()%4));
											glgen--;
										}
										else
										{
							...
							{
								world->GetBlock(x,(110 - y) + 48, z) = Netherrack::getID();
							}
						}
						else
						{
			...
			for (int z = 0; z < WORLD_SIZE; z++)
			{
				for (int x = 0; x < WORLD_SIZE; x++)
				{
					world->GetBlock(x,111,z) = IronBlock::getID();
					world->GetBlock(x,110,z) = IronBlock::getID();
				}
			}
			
			initQuartz(WORLD_SIZE, world);
			initLavaStuff(WORLD_SIZE, world);
			world->initWorldBlocksLight();
		}
		...
		// Generacion del portal al entrar al nether
		if(!portalCreate && nether)
		{
			// Variables
			int xz = WORLD_SIZE / 2;
			int yHeight = newPlayerSpawnPosition.y;
			int portalHeight = 4;
			int portalWidth = 2;
			
			// Verificar si está en un lugar cerrado y limpiar espacio
			for(int offsetZ = -1; offsetZ <= 1; offsetZ += 2){
				if(world->GetBlock(xz, yHeight, xz + offsetZ) != 0){
					for(int offsetY = -1; offsetY <= portalHeight; offsetY++){
						for(int offsetX = -1; offsetX <= portalWidth; offsetX++){
							world->GetBlock(newPlayerSpawnPosition.x + offsetX, newPlayerSpawnPosition.y + offsetY, newPlayerSpawnPosition.z + offsetZ) = 0;
						}
					}
				}
			}
			...
			for(int offsetY = -1; offsetY < portalHeight; offsetY++){
				for(int offsetX = -1; offsetX <= portalWidth; offsetX++){
					if(offsetY == -1 || offsetY == portalHeight - 1 || offsetX == -1 || offsetX == portalWidth){
						world->GetBlock(newPlayerSpawnPosition.x + offsetX, newPlayerSpawnPosition.y + offsetY, newPlayerSpawnPosition.z) = Obsidian::getID();
					}else{
						world->GetBlock(newPlayerSpawnPosition.x + offsetX, newPlayerSpawnPosition.y + offsetY, newPlayerSpawnPosition.z) = HellPortal::getID();
					}
				}
			}
