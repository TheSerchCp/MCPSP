...
{
	public:
		void initRandompMap(int worldSize, int worldHeight, int chunkSize, CraftWorld *world, bool makeDungeons,bool makeBonus, unsigned int seedII, int worldType, bool nether);
		
		// New chunk-based generation methods
		void generateChunk(WorldChunk* chunk, CraftWorld* world, unsigned int seed, int worldType, bool nether);
		void generateChunkTerrain(WorldChunk* chunk, int chunkWorldX, int chunkWorldY, int chunkWorldZ, unsigned int seed);
		void generateChunkFeatures(WorldChunk* chunk, CraftWorld* world, int chunkWorldX, int chunkWorldY, int chunkWorldZ);
		...
		void initNehterEnvironmentBlocks(int WORLD_SIZE, CraftWorld * world, int type);
		void initLavaStuff(int WORLD_SIZE, CraftWorld *world);
		void initNetherFortress(int WORLD_SIZE, CraftWorld *world, int estructura);
		void initQuartz(int WORLD_SIZE, CraftWorld *world);
		// END
